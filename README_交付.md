...
|-----|------|------|
| 1 | GET_INFO | 读模块信息 |
| 2 | READ_TID | 读 TID 区（默认 6 字，最多 31 字） |
| 3 | READ_USER | 读 USER 区（最多 31 字） |
| 4 | WRITE_USER | 写 USER 区（单段 ≤15 字可分段） |
| 5 | WRITE_EPC | 写 EPC 区（addr≥2，前两字 PC/CRC 禁写；总分段 ≤31 字，单段 ≤15 字） |
...
固件按 EPC C1G2 标准上限放行，越界写由标签自己返回错误码如实上报。

| 存储区 | Bank | 读上限 | 写上限 | 说明 |
|--------|------|--------|--------|------|
| RFU（保留/密码） | 0 | 4 字（8 字节） | **禁止** | 含 Kill/Access 密码，误写可能报废标签 |
| EPC | 1 | 31 字（62 字节） | 总分段 ≤31 字，单段 ≤15 字，addr≥2 | 前两字 PC/CRC 禁写；标签容量不足标签返回错误 |
| TID | 2 | 31 字 | 禁止（只读） | 出厂固化，标签本身拒写 |
| USER | 3 | 31 字 | 单段 ≤15 字可分段 | 标签容量看芯片型号 |

**说明**：
- 单次命令写入数据受 RxPDO 数据区 30 字限制，写 >30 字需发多次命令
- EPC 实际容量看标签芯片（常见 12/16 字节），写超过标签容量标签返回错误码，固件如实上报

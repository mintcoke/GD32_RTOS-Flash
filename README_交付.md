...
**输出格式**（DI(107..234)，每通道一段）：
```
[通道1 EPC状态][通道1 EPC N1字][通道1 USER状态][通道1 USER WORDS字]
[通道2 EPC状态][通道2 EPC N2字][通道2 USER状态][通道2 USER WORDS字]
[通道3 EPC状态][通道3 EPC N3字][通道3 USER状态][通道3 USER WORDS字]
```
- 每段前 1 字状态：0=成功，非 0=错误码（如 0x15 无标签）
- EPC 长度 N 由标签 PC 字自动决定（不同标签可能不同）
- 失败通道：状态写错误码，数据填 0，继续下一通道
- `DI(106)` = 总字节数

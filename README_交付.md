...
|-----|------|------|
| 1 | GET_INFO | 读模块信息 |
| 2 | READ_TID | 读 TID 区（默认 6 字，最多 122 字） |
| 3 | READ_USER | 读 USER 区（最多 122 字） |
| 4 | WRITE_USER | 写 USER 区（单次 ≤128 字，单段 ≤15 字可分段） |
...
| 33 | READ_EPC_USER_ALL | 一键读三通道 EPC+USER（ADDR=USER起始，WORDS=USER字数≤32） |
| 100 | RAW | 原始命令直发模块 |
...
单次命令传输量受 PDO 数据区限制（读响应 128 字、写请求 128 字），读操作另受 rfid_rx_buf 限制（≤122 字）。

| 存储区 | Bank | 读上限 | 写上限 | 说明 |
|--------|------|--------|--------|------|
| RFU（保留/密码） | 0 | 4 字（8 字节） | **禁止** | 含 Kill/Access 密码，误写可能报废标签 |
| EPC | 1 | 31 字（62 字节） | ≤31 字，addr≥2 | 前两字 PC/CRC 禁写 |
| TID | 2 | 122 字（244 字节） | 禁止（只读） | 出厂固化，标签本身拒写 |
| USER | 3 | 122 字（244 字节） | ≤128 字 | 标签容量看芯片型号 |
...
**说明**：
- 单次命令读写受 PDO 数据区 128 字限制（响应数据 DI(107..234)、写入数据 DO(4..131)）
- 读操作另受 rfid_rx_buf 缓冲区限制（≤122 字），大于 122 字的读会被拒
- EPC 受标准上限 31 字限制（PC 字 5 位编码）
- `SET_EPC_LEN` 的 `words` 是字数（1~31），不是字节数
- 写已不再分段，words 多少直接单次写多少

### 5.6 READ_EPC_USER_ALL 一键读三通道

一条命令依次读通道 1/2/3 的 EPC + USER 区，结果拼在 TxPDO 响应区返回。

**输入**：`ADDR`=USER 区起始地址（通常 0），`WORDS`=USER 区每通道读几字（1~32），`ANT` 不用。

**输出格式**（DI(107..234)，每通道固定长度 = 1 + 8 + 1 + WORDS 字）：
```
[通道1 EPC状态][通道1 EPC 8字][通道1 USER状态][通道1 USER WORDS字]
[通道2 EPC状态][通道2 EPC 8字][通道2 USER状态][通道2 USER WORDS字]
[通道3 EPC状态][通道3 EPC 8字][通道3 USER状态][通道3 USER WORDS字]
```
- EPC 固定 8 字（不足补 0，超出截断），保证每通道固定长度，PLC 可按固定偏移读
- 每段前 1 字状态：0=成功，非 0=错误码（如 0x15 无标签、0x09 读失败）
- 失败通道：状态写错误码，EPC/USER 数据填 0，继续下一通道
- `DI(106)` = 总字节数 = 3 × (1+8+1+WORDS) × 2

**PLC 解析**（WORDS=8 为例，每通道 18 字）：
- 通道1：DI(107)=EPC状态, DI(108..115)=EPC, DI(116)=USER状态, DI(117..124)=USER
- 通道2：DI(125)=EPC状态, DI(126..133)=EPC, DI(134)=USER状态, DI(135..142)=USER
- 通道3：DI(143)=EPC状态, DI(144..151)=EPC, DI(152)=USER状态, DI(153..160)=USER

...
void MainLoop(void)
{
    ...
    if(bInitFinished == FALSE)
    {
        ...
        /* FreeRun-Mode:  bEscIntEnabled = FALSE, bDcSyncActive = FALSE
           ...
           BUGFIX: The original condition
               (!bEscIntEnabled || !bEcatFirstOutputsReceived)
           becomes FALSE once the first SM event is received, after which
           only PDI_Isr() is expected to drive ECAT_Application().
           However, if the ESC interrupt stops firing (observed with
           SYNCTYPE_SM_SYNCHRON on TR8253 + GD32 via SPI — the EXTI
           on PB15 stops generating interrupts after ~36 events during
           the INIT→OP transition), the application never runs again.
           Result: PLC sees all-zero PDO data.

           Fix: when not in DC sync mode, always run the application
           from the main loop.  This provides a reliable fallback
           regardless of whether the ESC interrupt is active.  The
           DISABLE_ESC_INT/ENABLE_ESC_INT pair prevents concurrent
